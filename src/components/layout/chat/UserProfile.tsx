...
import { useAuth, useCurrentUser } from '@/hooks/auth/useAuth';
import { selectedGroupAtom } from '@/store/auth/auth';
...
const UserProfile = () => {
  ...
  const userRole = useMemo(() => {
    ...
    if (isGroupSelected) {
      return formatUserRole(selectedGroup.role?.toLowerCase());
    }
    return formatUserRole(user?.role?.default?.toLowerCase());
  }, [isSuperAdmin, selectedGroup, user]);
  ...
  const getUserName = () => {
    return user?.username || '홍길동';
  };

  const getUserCompany = () => {
    return user?.company || 'SK AX';
  };
    ...
    <div className='flex-shrink-0 px-5 py-5 relative' ref={menuRef}>
      <div className='flex items-center gap-3.5'>
        ...
        <div
          ...
          className='w-11 h-11 rounded-full bg-gradient-to-br from-neutral-200 to-neutral-300 flex items-center justify-center text-[15px] font-semibold text-neutral-700 shadow-sm cursor-pointer hover:from-neutral-300 hover:to-neutral-400 transition-all'
        >
          {getInitial()}
        </div>
